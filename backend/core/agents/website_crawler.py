...
def website_crawler_agent(state: WebsiteResearchState) -> dict:
    ...
    logger.info(f"[step-web-1][website_crawler] 收到爬取任务: {len(urls)} 个 URL | 爬取深度: {crawl_depth} | 最大页面数: {max_pages}")

    # 如果没有提供 URL，跳过爬取
    if not urls:
        logger.warning("[step-web-1][website_crawler] 未提供任何 URL，跳过爬取")
        return {
            "crawled_content": [],
            "failed_urls": [],
    ...
    logger.info(f"[step-web-1][website_crawler] 开始爬取: {urls}")
    # 调用 Crawl4AI 同步爬取服务
    ...
    logger.info(f"[step-web-1][website_crawler] 爬取完成: 成功 {len(succeeded)} 个, 失败 {len(failed)} 个")

    if succeeded:
        logger.info(f"[step-web-1][website_crawler] 成功 URL: {[r['url'] for r in succeeded]}")
    if failed:
        logger.warning(
            f"[step-web-1][website_crawler] 失败 URL: {[(r['url'], r['error']) for r in failed]}"
        )
    ...
    # 构建步骤描述
    step = f"[step-web-1][website_crawler]已爬取 {len(succeeded)}/{len(urls)} 个网站，共 {len(results)} 个页面"
    if failed:
